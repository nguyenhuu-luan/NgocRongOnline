...
public class ItemService {
    ...
    /**
     * Thử thêm ngẫu nhiên option "Sao Pha Lê" (ID 107) vào trang bị.
     * Hàm này sẽ được gọi khi vật phẩm rơi ra từ quái.
     * @param itemMap Vật phẩm trên bản đồ cần xử lý.
     */
    public void tryAddItemSpecialOption(ItemMap itemMap) {
        ...
        // Chỉ áp dụng cho trang bị (áo, quần, găng, giày, rada)
        if (itemMap.itemTemplate.type >= 0 && itemMap.itemTemplate.type <= 4) {
            // Kiểm tra xem trang bị này đã có sẵn option sao chưa để tránh trùng lặp
            boolean hasStarOption = false;
            for (Item.ItemOption opt : itemMap.options) {
                if (opt.optionTemplate.id == STAR_OPTION_ID) {
                    hasStarOption = true;
                    break;
                }
            }

            // Nếu trang bị chưa có sao và may mắn trúng tỷ lệ
            if (!hasStarOption && Util.isTrue(STAR_OPTION_RATE, 100)) {
                // Tạo giá trị ngẫu nhiên cho cấp sao (từ 1 đến 6)
                int randomStarLevel = Util.nextInt(1, 6);

                // Thêm option sao mới vào vật phẩm
                itemMap.options.add(new Item.ItemOption(STAR_OPTION_ID, randomStarLevel));
            }
        }
    }
