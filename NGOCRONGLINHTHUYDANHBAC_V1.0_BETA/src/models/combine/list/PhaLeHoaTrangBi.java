...
public class PhaLeHoaTrangBi {
    ...
    public static void phaLeHoa(Player player, int... numm) {
        if (player.combineNew != null && !Util.canDoWithTime(player.combineNew.lastTimeCombine, 500)) {
            return;
        }
        player.combineNew.lastTimeCombine = System.currentTimeMillis();        int n = 1;
        if (numm.length > 0) {
            n = numm[0];
        }
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (CombineUtil.isTrangBiPhaLeHoa(item)) {
                    ...
                    if (star < MAX_STAR_ITEM) {
                        ...
                        //fix tỉ lệ đập đồ từ 6->12s
                     /*note: phần này thực ra không gây lỗi, nó chỉ tăng
                     * độ khó đập đồ theo cấp số nhân tùy theo cấp sao
                     * càng cao thì tỉ lệ đập thành công càng thấp -> lâu vl
                     * nên bỏ luôn */
                      /*  int ratio = 1;
                        boolean succ = true;
                        if (optionStar != null) {
            ...
            if (item != null && fail > 0) {
                ...
                for (Item.ItemOption io : item.itemOptions) {
                    if (io.optionTemplate.id == 213) {
                        //fix lỗi tràn int = số âm
                        /*note: đây là nguyên nhân gây lỗi mất đồ khi đập
                        * việc gán số lần đập xịt vào biến fail khiến nó tăng
                        * nhanh và tràn số dẫn đến việc có khả năng xuất hiện giá trị âm
                        * nhưng fail lại đc định nghĩa là 1 biến giá trị INT (ko âm)
                        * -> tỉ lệ đập xịt nhiều -> khả năng lỗi cao
                        * -> bỏ gán hoặc bỏ luôn option hiển thị số lần xịt này vì đéo cần*/
                        //fail += io.param;
                        optionXit = io;
                        break;
                    }
